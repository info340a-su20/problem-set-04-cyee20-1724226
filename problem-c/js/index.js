...
let oppArray = huskyGames2016.map(extractOpponent);
...
let newArray = huskyGames2016.map(huskiesLost);
let lostGame = newArray.filter(function(n){
  return n != null;
});
...
let lostMap = huskyGames2016.map(extractResult);
let lostArray = lostMap.filter(function(n){
  return n != null;
});
...
let getFumble = function(game) {
  if (game.fumbles >= 1) {
    return game;
  }
};

let fumbleCount = huskyGames2016.map(getFumble);
let fumble = fumbleCount.filter(function(n) {
  if (n != null) {
    return n;
  }
});
...
let  mapArray = huskyGames2016.map(makeCombinedFilter(huskiesLost, getFumble));
let fumbledAndLostFilter = mapArray.filter(function(n){
  return n != null;
});
...
console.log(fumbledAndLostFilter);
