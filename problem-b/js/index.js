...
function compoundInterest(balance, rate, years) {
  return Math.exp(rate * years) * balance;
}
...
function getHighCard(pokerHand) {
  let highCard = pokerHand[0];
  for (let i = 1; i < pokerHand.length; i++) {
    if (highCard.rank < pokerHand[i].rank) {
      highCard = pokerHand[i];
    }
  }
  return highCard;
}
...
function isFlush(pokerHand) {
  let card = pokerHand[0].suit;
  for (let i = 1; i < pokerHand.length; i++) {
    if (card != pokerHand[i].suit) {
      return false;
    }
  }
  return true;
